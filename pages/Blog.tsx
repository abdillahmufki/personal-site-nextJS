      ...
      <div className="grid lg:grid-cols-3 place-items-center md:grid-cols-2 sm:grid-cols-1 gap-3">
        {data && Array.isArray(data) ? (
          data.length === 0 ? (
            ...
            data.map((post: Post) => <BlogPost key={post._id} post={post} />)
          )
        ) : (
          <div className="flex justify-center ">
            <p className="text-center dark:text-white text-gray-20">
              Loading...
            </p>
          </div>
        )}
...
const BlogPost: React.FC<BlogPostProps> = ({ post }) => (
  <Link href={`/post/${post.slug.current}`} key={post._id}>
    <a className="container mx-auto px-4 lg:max-w-7xl mt-5">
